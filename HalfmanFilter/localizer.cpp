...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
    ...
    get_grid_height_and_width(grid, height, width);
    if (height > 0 && width > 0) {
        newGrid = vector< vector <float> >(height, vector<float>(width, (1.0 / (height * width))));
    }
	return newGrid;
}
    ...
    if (height > 0 && width > 0) {
        ...
        for (long i = 0; i < height; i++) {
            ...
            for (long j = 0; j < width; j++) {
                ...
                newRow.push_back(beliefs[i][j] * p_hit * isColor + beliefs[i][j] * p_miss * (1 - isColor));
            }
	...
	float blurring) 
{
	...
	// your code here
    long height = 0, width = 0;
    get_grid_height_and_width(beliefs, height, width);
    if (height > 0 && width > 0) {
        newGrid = vector < vector <float> >(height, vector<float>(width, 0.0));
        for (long i = 0; i < height; i++) {
            for (long j = 0; j < width; j++) {
                newGrid[(i + dx + height) % height][(i + dy + width) % width] = beliefs[i][j];
            }
        }
    }
